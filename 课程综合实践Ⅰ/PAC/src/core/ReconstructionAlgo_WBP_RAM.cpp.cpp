...
#pragma omp critical
{
    plan_fft=fftwf_plan_dft_r2c_2d(Ny,Nx,(float*)bufc,reinterpret_cast<fftwf_complex*>(bufc),FFTW_ESTIMATE);//问题应该出在这个函数
...
//#pragma omp parallel for
    ...
    if(!skip_ctfcorrection) // read in defocus file for CTF correction
    {
        ...
        for(int n=0;n<stack_orig.getNz();n++)
        {
            ...
            {
                ctf_para[n].setAllCTFPara(defocus_tmp[1],defocus_tmp[2],defocus_tmp[3]-psi_deg,defocus_tmp[4],w_cos);   // 特别注意：目前的CTF估计结果取自CTFFIND4，是用原图（即未经旋转的图）估计的，因 
此对于重构旋转后的图，像散角（astig）也要旋转对应的角度！！！
            }
        }
        fflush(fdefocus);
        fclose(fdefocus);
...
//cout << "\t测试断点1" << endl;
                    ...
                    for (int zz = -int(h_tilt_max / 2); zz < int(h_tilt_max / 2); zz += defocus_step)
                    {
                        ...
                        memcpy(stack_corrected[n_z], image_now, sizeof(float) * daxiao);
                    }
