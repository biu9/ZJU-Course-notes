...
public:
    ...
    int readnPixels(void *buf, int nimage, int nline, int npixel,int n);
        ...
        void updateHeader();    // output the new header
        void computeHeader(float pix,bool is_image);    // compute & output the new header       
        void computeHeader_omp(float pix,bool is_image,int threads);    // compute & output the new header
        void computeHeader(float pix,bool is_image,float min_now,float max_now,float mean_now); // compute & output the new header

        int createMRC(float *data, int nx, int ny, int nz);
        int createMRC(short *data, int nx, int ny, int nz);
        int createMRC_empty(int nx, int ny, int nz, int mode);
        bool hasFile();
