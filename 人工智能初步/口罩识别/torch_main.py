...
for i in range(mask_num):
    sub_img = cv2.imread(data_path + "/image/mask/mask (" + str(i + 101) + ").jpg")
...
for i in range(nomask_num):
    sub_img = cv2.imread(data_path + "/image/nomask/nomask (" + str(i + 101)+ ").jpg")
    sub_img = cv2.cvtColor(sub_img, cv2.COLOR_RGB2BGR)
    ax = fig1.add_subplot(4, 4, (i + 1))
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_title("nomask (" + str(i + 1)+")")
    ax.imshow(sub_img)
...
train_data_loader, valid_data_loader = processing_data(data_path=data_path, height=160, width=160, batch_size=32)
modify_x, modify_y = torch.ones((32, 3, 160, 160)), torch.ones((28))
...
i=1
for i in range (3):
    img = Image.open('./test/test' + str(i+1) + '.jpg')
